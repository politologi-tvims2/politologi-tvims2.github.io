...
describe(lab2)
## let us center the variable lngdp2 (the mean value of the transformed variable is 0)
lab2$lngdp_new = lab2$lngdp2 - mean(lab2$lngdp2) 
describe(lab2$lngdp_new)
...
m3 <- lm(gove ~ va + rl + rq + ps + cc + lngdp_new, data = lab2)
summary(m3)
# interpret the coefficient estimates and their significance

# draw scatterplots to examine relationship types between the explanatory variables
pairs(lab2[,-c(1,5,8,10)])
# identify the approximate values of Pearson's correlation coefficients
# draw a more detailed graph
ggpairs(lab2[,-c(1,5,8,10)])
# correlation heatmap
ggcorr(lab2[,-c(1,5,8,10)], label = TRUE)
corrmatrix <- cor(lab2[,-c(1,5,8,10)])
corrmatrix 
...
summary(lm(va ~ rl + rq + ps + cc + lngdp_new, data=lab2))
summary(lm(rl ~ va + rq + ps + cc + lngdp_new, data=lab2))
summary(lm(rq ~ va + rl + ps + cc + lngdp_new, data=lab2))
summary(lm(ps ~ va + rl + rq + cc + lngdp_new, data=lab2))
summary(lm(cc ~ va + rl + rq + ps + lngdp_new, data=lab2))
summary(lm(lngdp_new ~ va + rl + rq + ps + cc, data=lab2))
...
vif <- vif(m3)
vif
1/(1 - summary(lm(va ~ rl + rq + ps + cc + lngdp_new, data=lab2))$r.sq)
...
m3_new <- lm(gove ~ va + rl + rq + ps + cc + lngdp_new, data=lab2[lab2$n>10,])
summary(m3_new)
